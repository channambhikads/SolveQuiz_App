...
function Welcome() {
  ...
  return (
    <main className="bg-primary1 h-screen w-screen flex xl:flex-row items-center justify-center gap-5">
      <Image preview={false} src={bg} height={519} width={672} />
      <div>
        ...
        <div className="flex flex-row items-center justify-center">
          <Link to="/register">
            <Button className="Button2" size="large" icon={<DoubleRightOutlined />} type="primary">
              Get started!
            </Button>
          </Link>
          <Link to="/login">
            <Button className="ml-2" size="large">
              Log in! 🚀
            </Button>
          </Link>
        </div>
